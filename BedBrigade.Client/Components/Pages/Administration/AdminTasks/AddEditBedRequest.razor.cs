    ...
    public partial class AddEditBedRequest : ComponentBase
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                _toastService?.Error("Error", "An error occurred while initializing the Bed Request editor.");
            }
        }
            ...
            if (BedRequestId.HasValue && BedRequestId.Value > 0)
            {
                if (_svcBedRequest != null)
                {
                    ...
                    else
                    {
                        ...
                        _toastService?.Error("Error", result.Message);
                ...
                else
                {
                    ...
                    _toastService?.Error("Error", "_svcBedRequest service is not available.");
                    Model = new BedBrigade.Common.Models.BedRequest { LocationId = LocationId };
                }
            }
            else

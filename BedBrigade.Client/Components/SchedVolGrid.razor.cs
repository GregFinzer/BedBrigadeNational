...
{
    public partial class SchedVolGrid : ComponentBase
    {
        // Data Services
        ...
        private bool IsVolunteerDialogVisible { get; set; } = false;
        private ResizeDirection[] SchedVolResizeDirections { get; set; } = new ResizeDirection[] { ResizeDirection.All };
        ...
        public Schedule? selectedGridObject { get; set; }

        protected DialogSettings DialogParams = new DialogSettings { Width = "800px", MinHeight="200px", EnableResize=true };

        // Event Volunteers Dialog Variables
        public string DialogTitle = "Test Title";
        ...
        private async Task LoadUserData()
        {
            ...
            {
                ToolBar = new List<string> { "Add", "Edit", "Delete", "Print", "Pdf Export", "Excel Export", "Csv Export", "Search", "Reset", "ColumnChooser" };
    ...
    new GridFilterOption() { ID= "all", Text= "All Schedules" },
        ...
        // Volunteer Info & Edit parts


        private void OpenVolunteerDialog(Schedule mySchedule)
        {
            selectedGridObject = mySchedule;
            this.IsVolunteerDialogVisible = true;
        }

        private void OnOverlayclick(OverlayModalClickEventArgs arg)
        {
            this.IsVolunteerDialogVisible = false;
        }

        private async Task CheckChildData(int updatedScheduleId)  // from Search Location Component
        { // update the current Grid Row only

            // get updated schedule
            var dataSchedule = await _svcSchedule.GetByIdAsync(updatedScheduleId);
            if (dataSchedule.Success && dataSchedule != null)
            {
                Schedule mySchedule = dataSchedule.Data;

                //Debug.WriteLine("Updated Schedule: " + mySchedule.ScheduleId.ToString());
            }
            
        } // check child component data        


        private MarkupString GetRegistrationBadge(Schedule mySchedule, string strObject)
        {            
            var strHtml = String.Empty;
            var sbHtml = new StringBuilder();
            string badgeClass = string.Empty;
            bool bDisplayBadge = true;
            var intRegValue = 0;
            var intMaxValue = 0;
            ...
            {
                case "Volunteer":
                    intRegValue = mySchedule.VolunteersRegistered;
                    intMaxValue = mySchedule.VolunteersMax;
                    break;
                case "Vehicle":
                    intRegValue = mySchedule.VehiclesDeliveryRegistered;
                    intMaxValue = mySchedule.VehiclesDeliveryMax;
                    break;
            } // Object

            switch (mySchedule.EventStatus)
            {
                case EventStatus.Scheduled:
                    if(intRegValue >= intMaxValue)
                    {
                        badgeClass = "success";
                    }
                    else
            ...
            {
                sbHtml.Append("<span ");
                sbHtml.Append("class='badge rounded-pill fw-bold bg-"); // bootstrap badge
                sbHtml.Append(badgeClass);
                sbHtml.Append("'>");
            }
            else
            {
            ...
            sbHtml.Append(intRegValue.ToString());
            sbHtml.Append("</span>");

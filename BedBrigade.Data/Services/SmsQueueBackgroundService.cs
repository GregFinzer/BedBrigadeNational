...
namespace JBadgerHVAC.Data.Services
{
    public class SmsQueueBackgroundService : BackgroundService
    {
        ...
        private async Task PerformMailMerge(SmsQueue message)
        {
            if (message.SignUpId.HasValue)
            {
                ...
                ServiceResponse<Volunteer> volunteerResponse = await _volunteerDataService.GetByIdAsync(signUpResponse.Data.VolunteerId);
                ...
                ServiceResponse<Location> locationResponse = await _locationDataService.GetByIdAsync(signUpResponse.Data.LocationId);
                ...
                ServiceResponse<Schedule> scheduleResponse = await _scheduleDataService.GetByIdAsync(signUpResponse.Data.ScheduleId);
        ...
        private bool TimeToSendMessages()
        {
            ...
            bool validDay = currentDate.DayOfWeek >= (DayOfWeek)_smsBeginDayOfWeek && currentDate.DayOfWeek <= (DayOfWeek)_smsEndDayOfWeek;
        ...
        private static async Task<int> LoadSmsConfigValue(string id)
        {
            var configResult = await _configurationDataService.GetByIdAsync(id);
